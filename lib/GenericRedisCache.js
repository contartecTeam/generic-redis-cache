...
class GenericRedisCache {
  ...
  */
  static async set(key, value = undefined, commands = redis.multi()) {
    try {
      ...
      if (key instanceof Object && !value) {
        ...
        if (idAttrs) {
          redisResponse = await this
            .setCache(idAttrs, key, commands)
        }
      }
      else {
        redisResponse = await this
          .setCache(key, value, commands)
      }
  ...
  */
  static async setCache(key, value, commands = redis.multi()) {
      ...
      if (objects.length) {
        ...
        for (const objectIndex in objects) {
          ...
          if (objectAttrs.length == 2 && objectAttrs[0] == 'key')
            await this.set(object.key, object.value, commands)
          else
            await this.set(object, null, commands)
        }
