...
class GenericRedisCache {
  ...
  */
  ...
  static async getDB() { return null }
    ...
    if (idAttrs) {
      ...
      if (objectTemp) {
        ...
        if (
          ...
          await this.onSave(idAttrs, objectTemp, oldCache, commands)
    ...
    try {
      ...
      if (key instanceof Object && !value) {
        redisResponse = await this
          .setObject(key, jsonPath, commands)
      }
      else {
        redisResponse = await this
          .setValue(key, value, jsonPath, commands)
      }
