...
*/
class GenericRedisCache {
  get KEY_NAME()             { return this.keyAttrs._keyName }
  get TYPE()                 { return this.keyAttrs._type }
  get ID()                   { return this.keyAttrs._id }
  ...
  static async onGet() { return null }
  ...
  */
  static async getKeyNames(keys) {
    ...
    else if (keys === undefined && this.ID.length == 1 && this.ID[0].undefinedValue) {
  ...
  static async get(key, params) {
    try {
      ...
      if (!cacheObject) {
        const dbObject = await this.onGet(key)
  ...
  */
  static async set(key, value = undefined, attr = GenericJSONCache.DEFAULT_PATH, commands = redis) {
    try {
      let redisResponse = null

      let savingVerification = true

      if (key instanceof Object && !value) {
        const idAttrs = await this._getIdAttr(key)

        if (idAttrs) {
          savingVerification = await this.onSavingVerify(idAttrs, key)

          if (savingVerification) {
            const oldCache = await this.getCache(idAttrs)

            const setCommand = await this
              .setCache(idAttrs, key, attr, commands)

            if (commands.constructor.name != 'RedisClient'
              || ( commands.constructor.name == 'RedisClient' && commands.command_queue && commands.command_queue._length > 0)) {
              await this.onSave(idAttrs, key, oldCache, commands)
        ...
        savingVerification = await this.onSavingVerify(key, value)

        if (savingVerification) {
          const setCommand = await this
            .setCache(key, value, attr, commands)

          if (commands.command_queue._length > 0) {
            await this.onSave(key, value, null, commands)
      ...
      if (commands.constructor.name == 'RedisClient' && savingVerification && redisResponse) {
      ...
      this.onSetError(e, key, value, attr)
    }
  }
  ...
  */
  static async setCache(key, value, attr = GenericJSONCache.DEFAULT_PATH, commands = redis.multi()) {
    const keyName = this
      .getKeyName(key)
      ...
      if (objects.length) {
        ...
        for (const objectIndex in objects) {
          ...
          if (objectAttrs.length == 2 && objectAttrs[0] == 'key')
            await this.set(object.key, object.value, GenericJSONCache.DEFAULT_PATH ,commands)
          else if (objectAttrs.length)
            await this.set(object, null, GenericJSONCache.DEFAULT_PATH, commands)
        }
  ...
  static async _getKeysNotCached(keys, objects) {
    ...
    if (keys) {
      if (keys.constructor.name == 'Object') {
        ...
        if (this.ID.length == 1 && keys[0] != null && (!objects.length || ( objects.length && objects[0].constructor.name == 'Object'))) {
