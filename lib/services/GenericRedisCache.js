...
class GenericRedisCache {
        ...
        idsIndexes.forEach((indexKeyName, index) =>  {
          ...
          if (paramKey) {
            ...
            if (typeof(objectValue) == 'string')
              ...
              if (!this.ATTRS.ids[index].undefinedValue)
                chars[indexKeyName] = this.ATTRS.idUndefined
              else
                chars[indexKeyName] =  this.ATTRS.ids[index].undefinedValue
            }
            else if (objectValue === null) {
              if (!this.ATTRS.ids[index].nullValue)
                chars[indexKeyName] = this.ATTRS.idNull
              else
                chars[indexKeyName] = this.ATTRS.ids[index].nullValue
            }
          }
          else {
            chars[indexKeyName] = this.ATTRS.ids[index].undefinedValue ?
              this.ATTRS.ids[index].undefinedValue :
  ...
  */
  static async getKeyNames(keys) {
    ...
    else if (keys === undefined && this.ATTRS.ids.length == 1 && this.ATTRS.ids[0].undefinedValue) {
    ...
    if (object) {
      ...
      this.ATTRS.ids.forEach(id => {
        if (object[id.id])
          ...
          tempObject[id.id] = id.undefinedValue ?
            id.undefinedValue :
            this.ATTRS.idUndefined
        }
      })
